...
from FasterTransformer.examples.pytorch.gpt.utils import (
    comm,
)  # isort: skip # yapf: disable # type: ignore
...
def download_convert(
    ...
    if s3:
        ...
        if os.path.exists(LOCAL_MODEL_PATH):
            print(f"[+] Path {LOCAL_MODEL_PATH} already exists, skipping download")
        else:
                ...
                try:
                    ...
                    if file_name not in downloaded_file_set:
                        ...
                        s3.download_file(
                            Bucket=parsed_path.netloc,
                            Key=file_key,
                            Filename=os.path.join(LOCAL_MODEL_PATH, file_name),
                        )
...
class MPTFTModelHandler:
    ...
    def __init__(
        ...
        if "gpt" in ckpt_config.keys():
            ...
            pipeline_para_size = ckpt_config.getint(
                "gpt", "pipeline_para_size", fallback=1
            )
            layernorm_eps = ckpt_config.getfloat("gpt", "layernorm_eps", fallback=1e-5)
    ...
    def _parse_model_request(self, model_request: Dict) -> Tuple[str, Dict]:
        if self.INPUT_KEY not in model_request:
            raise RuntimeError(f'"{self.INPUT_KEY}" must be provided to generate call')
    ...
    def _convert_kwargs(self, generate_inputs: List[str], generate_kwargs: Dict):
        ...
        generate_kwargs["temperature"] *= torch.ones(batch_size, dtype=torch.float32)
        repetition_penalty = generate_kwargs["repetition_penalty"]
        generate_kwargs["repetition_penalty"] = (
            None
            if repetition_penalty == 1.0
            else repetition_penalty * torch.ones(batch_size, dtype=torch.float32)
        )
        presence_penalty = generate_kwargs["presence_penalty"]
        generate_kwargs["presence_penalty"] = (
            None
            if presence_penalty == 0.0
            else presence_penalty * torch.ones(batch_size, dtype=torch.float32)
        )
        generate_kwargs["beam_search_diversity_rate"] *= torch.ones(
            batch_size, dtype=torch.float32
        )
        generate_kwargs["len_penalty"] *= torch.ones(
            size=[batch_size], dtype=torch.float32
        )
        generate_kwargs["min_length"] = int(generate_kwargs["min_length"]) * torch.ones(
            size=[batch_size], dtype=torch.int32
        )
        if generate_kwargs["random_seed"]:
            generate_kwargs["random_seed"] = torch.randint(
                0, 10000, size=[batch_size], dtype=torch.int64
            )
    ...
    def predict(self, model_requests: List[Dict]) -> List[str]:
        generate_inputs, generate_kwargs = self._parse_model_requests(model_requests)
        self._convert_kwargs(generate_inputs, generate_kwargs)

        start_ids = [
            torch.tensor(
                self.tokenizer.encode(c), dtype=torch.int32, device=self.device
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gpus", type=int, default=1, help="The number of gpus to use for inference."
    )

    parser.add_argument(
        "--force",
        action="store_true",
        help="Force conversion to FT even if some features may not work as expected in FT",
    )
    ...
    if not comm.is_model_parallel_initailized():
        ...
        comm.initialize_model_parallel(tensor_para_size=args.gpus, pipeline_para_size=1)
    ...
    model_handle = MPTFTModelHandler(
        args.name_or_dir,
        args.ft_lib_path,
        args.inference_data_type,
        args.int8_mode,
        args.gpus,
    )
